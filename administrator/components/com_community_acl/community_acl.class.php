...
class CB_config extends JObject
{
	...
	function storeData($userid, $cb_infos){
		...
		for($i=0; $i<$count; $i++){
			$query = "INSERT INTO `#__community_acl_users` " .
					 "  SET `user_id` = '" . $userid . "'" .
					 "	  , `group_id` = '". $cb_infos['cacl_group_id'][$i] . "'" .
					 "	  , `role_id` = '" . $cb_infos['cacl_role_id'][$i] . "'" .
					 "	  , `function_id` = '" . $cb_infos['cacl_func_id'][$i] . "'" .
					 "	  , `redirect_front` = '" . $cb_infos[redirect_url] . "'" .
					 "	  , `redirect_admin` = '" .$cb_infos[admin_redirect_url] . "'" 
					 ;
			$this->_db->setQuery($query);
			$this->_db->query();
		}
		...
		# - 2. Store in ACL_cb_groups
		 for($i=0; $i<$count; $i++){
			$query = "INSERT INTO `#__community_acl_cb_groups` " .
				 "	  SET `id` = '". $cb_infos['cacl_group_id'][$i] . "'" .
				 "	  , `name` = ''"  .
				 "	  , `description` = ''"  .
				 "	  , `enabled` = '1'"  .
				 "	  , `dosync` = ''"  .
		...
		for($i=0; $i<$count; $i++){
			$query = "INSERT INTO `#__community_acl_cb_roles` " .
				 "	  SET `id` = '" . $cb_infos['cacl_role_id'][$i] . "'" .
				 "	  , `group_id` = '". $cb_infos['cacl_group_id'][$i] . "'" .
				 "	  , `name` = ''"  .
				 "	  , `description` = ''"  .
				 "	  , `ordering` = ''"  .
	...
	# - 4. Store in ACL_cb_
		for($i=0; $i<$count; $i++){
			$query = "INSERT INTO `#__community_acl_cb_functions` " .
				 "	  SET `id` = '" . $cb_infos['cacl_func_id'][$i] . "'" .
				 "	  , `group_id` = '". $cb_infos['cacl_group_id'][$i] . "'" .
				 "	  , `name` = ''"  .
				 "	  , `description` = ''"  .
				 "	  , `j_group_id` =	'' ".
...
{	
	...
	function store(){		
		...
		foreach ($vars as $v) {
				...
				$query = "INSERT INTO `#__community_acl_config` (name, value) "
						."VALUES ('".$v."', '".$this->$v."')";
				$this->_db->setQuery($query);
				$this->_db->query();
		}
...
class CACL_role extends JTable
{
	...
	function check()
	{
		...
		// check for unique group name
		$query = 'SELECT name' .
				' FROM `#__community_acl_roles`  WHERE `group_id` = \''. (int)$this->group_id .'\' ';
		if ($this->id) {
...
class CACL_syncronize extends JObject {
	...
	function sync_cacl_access_func($tid){
		...
		$query = 'DELETE FROM `#__community_acl_content_actions`'
		. ' WHERE `func_id` = \''. $func_id_d .'\''
		;
		$this->dst_site->_site_db->setQuery( $query );
		$this->dst_site->_site_db->query( );
	...
	function sync_cacl_access($tid){
		...
		. ' WHERE `group_id` = \''. $group_id_d .'\' AND `role_id` = \''. $role_id_d .'\''
		;
		$this->dst_site->_site_db->setQuery( $query );
		$this->dst_site->_site_db->query( );
	...
	function sync_cacl_func($tid){
		...
		if ((int)$this->src_site->_site_db->loadResult() < 1) {
			...
			$query = 'UPDATE `#__community_acl_functions` SET `dosync` = 0 '  
			. ' WHERE `id` = \''. $id_d .'\' '
			;
			$this->dst_site->_site_db->setQuery( $query );
			$this->dst_site->_site_db->query( );
	...
	function sync_cacl_role($tid){
		...
		if ((int)$this->src_site->_site_db->loadResult() < 1) {
			$id_d = $this->get_id($t, $this->main_site, $this->src_site, $this->dst_site, $tid);
			$query = 'UPDATE `#__community_acl_roles` SET `dosync` = 0 '  
			. ' WHERE `id` = \''. $id_d .'\' '
			;
			$this->dst_site->_site_db->setQuery( $query );
			$this->dst_site->_site_db->query( );
	...
	function sync_cacl_group_delete($tid){
		...
		$query = 'DELETE FROM `#__community_acl_access`'
		. ' WHERE `group_id` = \''. $group_id .'\' '.(count($rid) > 0?' OR `role_id` IN ( '.implode(',', $rid).' )': '')
		;
		$this->dst_site->_site_db->setQuery( $query );
		$this->dst_site->_site_db->query( );
	...
	function sync_cacl_group($tid){
		...
		if ((int)$this->src_site->_site_db->loadResult() < 1) {
			$id_d = $this->get_id($t, $this->main_site, $this->src_site, $this->dst_site, $tid);
			$query = 'UPDATE `#__community_acl_groups` SET `dosync` = 0 '  
			. ' WHERE `id` = \''. $id_d .'\' '
			;
			$this->dst_site->_site_db->setQuery( $query );
			$this->dst_site->_site_db->query( );
	...
	function sync_row(&$sync_tbl, &$main_site, &$src_site, &$dst_site, $src_id, $no_key=false, $const = array()) {
		...
		$query = "SELECT dst_field_id FROM `#__community_acl_sync` " .
				" WHERE `src_site_id` = '". $src_site->id ."'".
				"	AND `dst_site_id` = '". $dst_site->id ."'".
				"	AND `tablename` = '". $sync_tbl->table ."'".
				"	AND `fieldname` = '". $sync_tbl->fields['key'][0] ."'".
				"	AND `src_field_id` = '". $src_id ."'";
		...
		if (!$dst_id && count($sync_tbl->fields['unique']) > 0) {
			$from_unique = true;
			$query = "SELECT `". implode('`, `', $sync_tbl->fields['unique']) ."` FROM `". $sync_tbl->table ."` " .
					" WHERE  `". $sync_tbl->fields['key'][0] ."` = ". $src_id;
		...
		$query = "SELECT {$fields} FROM `". $sync_tbl->table ."` " .
				" WHERE `". $sync_tbl->fields['key'][0] ."` = ". $src_id;
		$src_site->_site_db->setQuery($query);
		...
		if (is_object($row)){
			...
			if (count($sync_tbl->fields['linked']) > 0 ) {
				...
				foreach($sync_tbl->fields['linked'] as $name=>$details) {
					$query = "SELECT dst_field_id FROM `#__community_acl_sync` " .
						" WHERE `src_site_id` = '". $src_site->id ."'".
						"	AND `dst_site_id` = '". $dst_site->id ."'".
						"	AND `tablename` = '". $details['table'] ."'".
						"	AND `fieldname` = '". $details['key'] ."'".
						"	AND `src_field_id` = '". $row->$name ."'";
					$main_site->_site_db->setQuery($query);
					$dst_id_link = (int)$main_site->_site_db->loadResult();

					if ($dst_id_link) {
						$query = "SELECT COUNT(*) FROM `". $details['table'] ."` " .
								" WHERE  `". $details['key'] ."` = ". $row->$name;
					...
					if (!$dst_id_link && count($details['unique']) > 0) {
						$from_unique = true;
						$query = "SELECT `". implode('`, `', $details['unique']) ."` FROM `". $details['table'] ."` " .
								" WHERE  `". $details['key'] ."` = ". $row->$name;
						$src_site->_site_db->setQuery($query);			
			...
			if ($dst_id) {
				...
				if ($from_unique){
					...
					$query = "INSERT INTO `#__community_acl_sync` " .
					" SET `src_site_id` = '". $src_site->id ."'".
					"	, `dst_site_id` = '". $dst_site->id ."'".
					"	, `tablename` = '". $sync_tbl->table ."'".
					"	, `fieldname` = '". $sync_tbl->fields['key'][0] ."'".
					"	, `src_field_id` = '". $src_id ."'".
					"	, `dst_field_id` = '". $dst_id ."'";
					$main_site->_site_db->setQuery($query);

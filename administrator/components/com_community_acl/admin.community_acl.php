...
define ('_COMMUNITY_ACL_COMP_NAME', 'Community ACL ver 1.0.16');
...
{
	##-- GROUPS -------------
	case 'list_groups': 	showGroups($option); break;		
	case 'group_add': 		editGroup(false); break;
	case 'group_edit': 		editGroup(true); break;		
	case 'group_save':
	case 'group_apply':		saveGroup( ); break; 		
	case 'group_publish': 	publishItem( '#__community_acl_groups', $cid, 1 ); break;
	case 'group_unpublish':	publishItem( '#__community_acl_groups', $cid, 0 ); break;	
	case 'group_delete': 	deleteItem( '#__community_acl_groups', $cid );	break;

	case 'group_sync': 		changeItem( '#__community_acl_groups', $cid, 1 );	break;
	case 'group_unsync': 	changeItem( '#__community_acl_groups', $cid, 0 );	break;
	
	##-- ROLES --------------
	case 'list_roles': 		showRoles($option); break;
	case 'role_add': 		editRole(false); break;
	case 'role_edit': 		editRole(true); break;	
	case 'role_save':
	case 'role_apply':		saveRole( ); break;
	
	case 'role_orderup': 		orderItem( '#__community_acl_roles', 'group_id', $cid[0], -1); break;
	case 'role_orderdown': 		orderItem( '#__community_acl_roles', 'group_id', $cid[0], 1); break;
	case 'role_saveorder': 		saveOrder( $cid, '#__community_acl_roles', 'group_id' ); break;
	
	case 'role_publish': 	publishItem( '#__community_acl_roles', $cid, 1 ); break;
	case 'role_unpublish':	publishItem( '#__community_acl_roles', $cid, 0 ); break;	
	case 'role_delete': 	deleteItem( '#__community_acl_roles', $cid );	break;
	
	case 'role_sync': 		changeItem( '#__community_acl_roles', $cid, 1 );	break;
	case 'role_unsync': 	changeItem( '#__community_acl_roles', $cid, 0 );	break;
	
	##-- FUNCTIONS --------------
	case 'list_functions': 		showFunctions($option); break;
	case 'function_add': 		editFunction(false); break;
	case 'function_edit': 		editFunction(true); break;	
	case 'function_save':
	case 'function_apply':		saveFunction( ); break;
	
	case 'function_orderup': 		orderItem( '#__community_acl_functions', 'group_id', $cid[0], -1); break;
	case 'function_orderdown': 		orderItem( '#__community_acl_functions', 'group_id', $cid[0], 1); break;
	case 'function_saveorder': 		saveOrder( $cid, '#__community_acl_functions', 'group_id' ); break;
	
	case 'function_publish': 	publishItem( '#__community_acl_functions', $cid, 1 ); break;
	case 'function_unpublish':	publishItem( '#__community_acl_functions', $cid, 0 ); break;	
	case 'function_delete': 	deleteItem( '#__community_acl_functions', $cid );	break;
	
	case 'function_sync': 		changeItem( '#__community_acl_functions', $cid, 1 );	break;
	case 'function_unsync': 	changeItem( '#__community_acl_functions', $cid, 0 );	break;
	
	case 'set_functions': setFunctions($cid[0]);	break;
	case 'apply_functions': 
	case 'save_functions': saveFunctions();	break;
	
	##-- ACCESS -----------------
	case 'show_access': if ($mode == 'role_id') showAccess(0, $cid[0]); else showAccess($cid[0], 0);	break;
	case 'apply_access': 
	case 'save_access': saveAccess(); break;
	case 'delete_users_access': if ($mode == 'role_id') showUserAccessR(0, $cid[0]); else showUserAccessR($cid[0], 0); break;
	case 'delete_users_access_save': saveUserAccessR($cid); break;
	
	case 'add_users_access': if ($mode == 'role_id') showUserAccessA(0, $cid[0]); else showUserAccessA($cid[0], 0); break;
	case 'add_users_access_save': saveUserAccessA($cid); break;
	...
	case 'manage_users': 
		...
		left_menu_footer();
		break;
	case 'assign_users': showAssignUsers($option, $cid); break;
	case 'unassign_users': showUnassignUsers($option, $cid); break;
	case 'save_assign': saveAssignUsers($option, $cid); break;
	case 'unassign_save': saveUnassignUsers($option, $cid); break;
		
	
	#-- SITES ------------------
	case 'list_sites': 	showSites($option); break;
	case 'site_edit': 	editSites(true); break;
	case 'site_add': 	editSites(false); break;
	
	case 'site_publish': 	publishItem( '#__community_acl_sites', $cid, 1 ); break;
	case 'site_unpublish':	publishItem( '#__community_acl_sites', $cid, 0 ); break;	
	case 'site_delete': 	deleteItem( '#__community_acl_sites', $cid );	break;
	
	case 'apply_sites': 
	case 'save_sites': saveSites();	break;
	case 'check_db': cacl_check_db(); break;
	
	case 'set_main': setMainSite($cid[0]); break;		
	
	##-- CONFIG ----------------
	case 'config': showConfig(); break;
	case 'apply_config':
	case 'save_config': saveConfig(); break;
	case 'synchronize': cacl_html::synchronize(); break;
	case 'do_synchronize':
		doSynchronization();
		break;
	##-- LANGUAGE --------------------
	
	case 'language': case 'language_files': case 'edit_language': case 'save_language': case 'apply_language': case 'cancel_language': case 'checkin_language': case 'checkout_language': case 'language_publish': case 'remove_language': case 'language_unpublish':
		left_menu_header();
		switch($task) {
			case 'language': 
	...
	case 'fixit': fixIt(); break;
	case 'about': cacl_html::about(_COMMUNITY_ACL_COMP_NAME);break;
	case 'help': cacl_html::help(); break;
	case 'disclaimer': cacl_html::disclaimer(); break;
	case 'changelog': cacl_html::changelog(); break;
	#- Kobby Created the hacks
	case 'hacks': cacl_html::hacks(); break;
		...
		$comprofiler_filepath = $frontendPath."com_comprofiler/plugin/user/index.html";			
				switch($step){
					case 1:
						...
						# Update Community ACL tables
						caclInstaller::updateCACL_tables();
						break;
					case 2:
...
function showUserAccessA($group_id=0, $role_id=0) {
	...
	$query = "SELECT b.*, `c`.`name` AS `groupname` FROM `#__users` AS b LEFT JOIN `#__core_acl_aro_groups` AS c ON `b`.`gid` = c.id WHERE b.id NOT IN ('". implode("','", $users) ."') ORDER BY b.name ASC";
...
function showAssignUsers($option, $cid) {
	...
	$query = 'SELECT id, CONCAT(name, \' (\', username, \')\') AS name '
	. ' FROM `#__users`'
...
function saveConfig() {
	...
	$cb_config->storeData($cb_id, $post);
...
function showConfig() {
	...
	$query = 'SELECT DISTINCT a.id, a.*, e.name AS g_name, f.name AS r_name, g.name AS f_name'
...
function setFunctions($id=0){
	...
	$query = 'SELECT `name`, `value`' .
				' FROM `#__community_acl_function_access`' .
				' WHERE `option` = \'###\' AND `func_id` = \''.$id.'\'';
	$db->setQuery($query);
	$tmp = $db->loadObjectList();
	...
	$query = 'SELECT b.id AS c_id , b.name AS title, a.*' .
			' FROM #__community_acl_function_access AS a' .
			' LEFT JOIN #__components AS b ON b.option = a.option' .
			" WHERE a.func_id = '".$id."'  AND a.option <> '###' AND (b.parent = 0 OR a.option IN ('com_sections', 'com_frontpage', 'com_categories', 'com_trash'))" .			
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT grouping AS n' .
			' FROM #__community_acl_function_access ' .
			' WHERE func_id = \''.$id.'\' AND `option` <> \'###\'  ' .			
			' GROUP BY grouping' .
			' ORDER BY grouping';
	$db->setQuery($query);
	...
	$query = 'SELECT a.id, a.title, a.section, b.title AS stitle' .
			' FROM `#__categories` AS a LEFT JOIN `#__sections` AS b ON b.id = a.section ' .
	...
	$query = 'SELECT a.id, a.title, a.sectionid, a.catid, b.title AS stitle, c.title AS ctitle ' .
			' FROM `#__content` AS a LEFT JOIN `#__sections` AS b ON b.id = a.sectionid LEFT JOIN `#__categories` AS c ON c.id = a.catid ' . 
			' ORDER BY a.sectionid, a.catid, a.ordering, a.title ';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__sections AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'section\' AND a.action = \'add\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__sections AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'section\' AND a.action = \'edit\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__sections AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'section\' AND a.action = \'publish\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__categories AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'category\' AND a.action = \'add\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__categories AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'category\' AND a.action = \'edit\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__categories AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'category\' AND a.action = \'publish\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__content AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'content\' AND a.action = \'add\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__content AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'content\' AND a.action = \'edit\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT a.item_id AS value, b.title' .
			' FROM #__community_acl_content_actions AS a' .
			' LEFT JOIN #__content AS b ON b.id = a.item_id' .
			' WHERE a.item_type = \'content\' AND a.action = \'publish\'' .
			' AND a.func_id = '.$id.
			' ORDER BY a.id';
	$db->setQuery($query);
...
function showAccess($group_id=0, $role_id=0) {
	...
	$query = 'SELECT a.value, b.title, b.published' .
			' FROM #__community_acl_access AS a' .
			' LEFT JOIN #__sections AS b ON b.id = a.value' .
			' WHERE a.option = \'com_sections\' AND a.name = \'cid\'' .
			' AND a.group_id = '.$group_id.' AND a.role_id = '.$role_id .
			' ORDER BY a.id';
	$db->setQuery($query);
	$lists['sections'] = $db->loadObjectList();

	$query = 'SELECT b.id, b.title, b.published ' .
			' FROM `#__sections` AS b ';
	$db->setQuery($query);
	$lists['sections_arr'] = $db->loadObjectList();
	
	$query = 'SELECT a.value, b.title, b.published, c.title AS section_name' .
			' FROM #__community_acl_access AS a' .
			' LEFT JOIN #__categories AS b ON b.id = a.value' .
			' LEFT JOIN #__sections AS c ON b.section = c.id' .
			' WHERE a.option = \'com_categories\' AND a.name = \'cid\'' .
			' AND a.group_id = '.$group_id.' AND a.role_id = '.$role_id .
			' ORDER BY b.section, a.id';
	$db->setQuery($query);
	$lists['categories'] = $db->loadObjectList();
	
	$query = 'SELECT b.id, b.title, b.published, c.title AS section_name' .
			' FROM #__categories AS b ' .
			' LEFT JOIN #__sections AS c ON b.section = c.id';
	$db->setQuery($query);
	$lists['categories_arr'] = $db->loadObjectList();

	$query = 'SELECT a.value, b.title, b.state AS published, c.title AS section_name, d.title AS cat_name, e.name AS author_name' .
			' FROM #__community_acl_access AS a' .
			' LEFT JOIN #__content AS b ON b.id = a.value' .
			' LEFT JOIN #__sections AS c ON b.sectionid = c.id' .
			' LEFT JOIN #__categories AS d ON b.catid = d.id' .
			' LEFT JOIN #__users AS e ON b.created_by = e.id' .
			' WHERE a.option = \'com_content\' AND a.name = \'cid\'' .
			' AND a.group_id = '.$group_id.' AND a.role_id = '.$role_id .
			' ORDER BY a.id';
	$db->setQuery($query);
	$lists['contents'] = $db->loadObjectList();	
	
	$query = 'SELECT b.id, b.title, b.state AS published, c.title AS section_name, d.title AS cat_name, e.name AS author_name' .
			' FROM #__content AS b ' .
			' LEFT JOIN #__sections AS c ON b.sectionid = c.id' .
			' LEFT JOIN #__categories AS d ON b.catid = d.id' .
			' LEFT JOIN #__users AS e ON b.created_by = e.id';			 
	...
	$query = 'SELECT b.id AS value , b.name AS title, a.isfrontend, a.isbackend, a.option' .
			' FROM #__community_acl_access AS a' .
			' LEFT JOIN #__components AS b ON b.id = a.value' .
			' WHERE a.name = \'###\' AND a.option <> \'menu\' AND (b.parent = 0 OR a.option IN (\'com_sections\', \'com_frontpage\', \'com_categories\', \'com_trash\' )) ' .
			' AND a.group_id = '.$group_id.' AND a.role_id = '.$role_id .
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT b.*, b.id AS value , b.name AS mtitle, a.isfrontend, a.isbackend, d.name AS parent_name, c.title AS menu_name' .
			' FROM #__community_acl_access AS a' .
			' LEFT JOIN #__menu AS b ON b.id = a.value' .
			' LEFT JOIN #__menu_types AS c ON c.menutype = b.menutype' .
			' LEFT JOIN #__menu AS d ON d.id = b.parent' .
			' WHERE a.name = \'###\' AND a.option = \'menu\'' .
			' AND a.group_id = '.$group_id.' AND a.role_id = '.$role_id .
			' ORDER BY c.menutype, b.parent, b.ordering';
	$db->setQuery($query);
	...
	if($mode == 'role_id'){	
		...
		$query = 'SELECT redirect_FRONT AS front_redirect, redirect_ADMIN AS front_admin FROM #__community_acl_roles WHERE id = ' . $id;
		$db->setQuery($query);
		$result = $db->loadObjectList();
	}elseif($mode == 'group_id'){	
		# - Get the front_redirect and backend redirect from DB
		$query = 'SELECT redirect_URL_FRONT AS front_redirect, redirect_URL_ADMIN AS front_admin FROM #__community_acl_groups WHERE id = ' . $id;
		$db->setQuery($query);
		$result = $db->loadObjectList();
	}
	...
	if($result){
		//print_r($query);die();
		$lists['frontend_redirect'] = $result[0]->front_redirect;
		$lists['backend_redirect'] 	= $result[0]->front_admin;		
	}
	...
	$query = 'SELECT b.*, b.id AS value, b.name AS title, d.name AS parent_name, c.title AS menu_name ' .
			' FROM `#__menu` AS b ' .
			' LEFT JOIN `#__menu_types` AS c ON c.menutype = b.menutype ' .
			' LEFT JOIN `#__menu` AS d ON d.id = b.parent ' .
			' WHERE b.published <> -2 ' .
	...
	$query = 'SELECT b.id AS value , b.title, b.position, b.module, b.published' .
			' FROM #__community_acl_access AS a' .
			' LEFT JOIN #__modules AS b ON b.id = a.value' .
			' WHERE a.name = \'@@@\' ' .
			' AND a.group_id = '.$group_id.' AND a.role_id = '.$role_id .
			' ORDER BY a.id';
	$db->setQuery($query);
	...
	$query = 'SELECT m.*, m.name AS title, b.title AS mtitle' .
			' FROM #__menu m LEFT JOIN `#__menu_types` AS b ON b.menutype = m.menutype ' .
			' WHERE m.published != -2 ' .
...
function showGroups($option) {
	...
	$query = "SELECT * FROM `#__community_acl_groups` ";
...
function showRoles($option) {
	...
	$query = "SELECT a.*, b.name AS group_name FROM `#__community_acl_roles` AS a LEFT JOIN `#__community_acl_groups` AS b ON b.id = a.group_id ".($group_id > 0?" WHERE a.group_id = '{$group_id}' ":'') . " ORDER BY a.group_id, a.ordering";
	$db->setQuery( $query, $pageNav->limitstart, $pageNav->limit );
	$rows = $db->loadObjectList();
	if ($db->getErrorNum()) {
		echo $db->stderr();
...
function editRole($edit ) {
	...
	. ' FROM #__community_acl_roles'
	. ($row->group_id > 0? ' WHERE group_id = '.$db->Quote($row->group_id): '')
	. ' ORDER BY ordering'
	;
	if ($edit) {
...
function showFunctions($option) {
	...
	$query = "SELECT * FROM `#__community_acl_functions` ".($group_id > 0?" WHERE `group_id` = '{$group_id}' ":'') . " ORDER BY ordering";
	$db->setQuery( $query, $pageNav->limitstart, $pageNav->limit );
	$rows = $db->loadObjectList();
	if ($db->getErrorNum()) {
		echo $db->stderr();
	...
	if (count( $cid )) {
		...
		switch ($table) {
			case '#__community_acl_groups':							
				...
				$query = 'DELETE FROM `#__community_acl_roles`'
				. ' WHERE `group_id` IN ( '.$cids.' )'
				;
				$db->setQuery( $query );
				if (!$db->query()) {
				...
				$query = 'DELETE FROM `#__community_acl_access`'
				. ' WHERE `group_id` IN ( '.$cids.' ) '.(count($rid) > 0?' OR `role_id` IN ( '.implode(',', $rid).' )': '')
				;
				$db->setQuery( $query );
				if (!$db->query()) {
					JError::raiseError( 500, $db->stderr() );
				...
				$query = 'DELETE FROM `#__community_acl_users`'
				. ' WHERE `group_id` IN ( '.$cids.' ) '.(count($rid) > 0?' OR `role_id` IN ( '.implode(',', $rid).' )': '')
				;
				$db->setQuery( $query );
				if (!$db->query()) {
			...
			case '#__community_acl_roles':
				...
				break;
			case '#__community_acl_functions':
				$query = 'DELETE FROM `#__community_acl_function_access`'
				. ' WHERE `func_id` IN ( '.$cids.' ) '
				;
				$db->setQuery( $query );
				if (!$db->query()) {
					JError::raiseError( 500, $db->stderr() );

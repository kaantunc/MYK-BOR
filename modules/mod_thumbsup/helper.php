...
//				echo '</pre>';
				$pos = strpos($row->introtext, "{link}");
				if ( $pos !== FALSE){
					$posEnd = strpos($row->introtext, "{/link}");
					$lists[$i]->link = trim(substr($row->introtext, $pos+6, ($posEnd-$pos-6)));
				}else
			...
			if($imageName=='' && $pic_default) $imageName=$pic_default;
			...
			else $docroot="&docroot=".JPATH_SITE;
			
			if(trim($imageName) != '' && substr($matches[1][0],0,4)=="http" && !file_exists(JPATH_SITE.DS."images".DS."stories".DS.trim($imageName))) {
                    ...
                    } elseif(substr($matches[1][0],0,4)=="http" && file_exists(JPATH_SITE.DS."images".DS."stories".DS.trim($imageName))) {
                   $image_path=DS."images".DS."stories".DS.str_replace("%20","",trim($imageName));
			...
			$imgsmall="<img src=\"modules/mod_thumbsup/image.php/".$imageName."?width=".$pic_width."&height=".$pic_height."&".$cropratio."&".$color."&image=".$image_path.$docroot."\" alt=\"".$imageName."\" align=\"".$pic_align."\" hspace=\"10\" vspace=\"5\" />";
			//print $imgsmall;
            $imgbig="<img src=\"modules/mod_thumbsup/image.php/".$imageName."?width=200&".$color."&image=".$image_path.$docroot."\" alt=\"".$imageName."\" align=\"".$pic_align."\" hspace=10 vspace=10 />";
            }      	

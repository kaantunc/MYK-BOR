...
//require_once( JPATH_COMPONENT.DS.'helpers'.DS.'addrow.php');
...
// varsayilan olarak tr ve eng i kaydet
		
		$db =& JFactory::getDBO();
		$query = "SELECT id FROM #__chrono_contact";
		$db->setQuery($query);
		$formlar = $db->loadObjectList();		

		$query = "SELECT form_id FROM #__chrono_contact_plugins WHERE `name` = 'cf_multi_language' AND `event` = 'ONLOAD'";
		$db->setQuery($query);
		$plugins = $db->loadObjectList();
		$plg_form_ids = '';
		foreach($plugins as $plg_id_row){
			//echo "-$plg_form_ids-<br>";
			$plg_form_ids = $plg_form_ids . $plg_id_row->form_id . ',';
		}
		
		foreach($formlar as $form_row){
			
			//echo "form id: $form_row->id";
			
			if(strstr($plg_form_ids,$form_row->id)===FALSE){
				
				//echo " -> FALSE";
												
				$query = "INSERT INTO `#__chrono_contact_plugins` (
					`id` ,
					`form_id` ,
					`name` ,
					`event` ,
					...
					VALUES (
					NULL , '$form_row->id', 'cf_multi_language', 'ONLOAD', 'languages=en-GB,tr-TR\ndefault_language=tr-TR\nextra6=\nextra7=\nextra8=\nextra9=\nextra10=', '', '', '', '', '', '', '', '', '', ''
					);";
		...
		// ACTIVATE cf_multi_language PLUGIN
		$query = "SELECT id,paramsall FROM #__chrono_contact";
		$db->setQuery($query);
		$formlar = $db->loadObjectList();
		...
		foreach($formlar as $form){
			...
			//echo "-".substr($form->paramsall, strpos($form->paramsall, 'plugins=')+strlen('plugins='),2)."-";
			
//				echo "-<pre>";
//				print_r(substr($form->paramsall, strpos($form->paramsall, '=')+1,1));
//				echo "</pre>-";
			
			if(strpos($form->paramsall,"cf_multi_language")===FALSE){
				
//				echo "-<pre>";
//				print_r(substr($form->paramsall, strpos($form->paramsall, '=')+1,1));
//				echo "</pre>-";

				if(substr($form->paramsall, strpos($form->paramsall, 'plugins=')+strlen('plugins='),2)!="cf"){
					$form->paramsall = str_replace('plugins=', 'plugins=cf_multi_language', $form->paramsall);
				}
				else{
					$form->paramsall = str_replace('plugins=', 'plugins=cf_multi_language,', $form->paramsall);
				}
			
				//echo "-$form->paramsall-<br />";
	
				$query = "UPDATE `#__chrono_contact` SET `paramsall` = '$form->paramsall' WHERE `id`=$form->id";
				$db->setQuery($query);
				$result = $db->query();
				//echo "-$query-";
				if($result===FALSE){
		...
		// END cf_multi_language plugin activation
...
function save_form_wizard( $option ){
	...
	$mainframe->redirect( "index2.php?option=$option", JText::_('Form Saved, you can Open form to edit other settings now!') );

...
var CFSPAN = new Class({
	...
	createElement: function(){
		this.thespan = new Element('span', {'name':this.name}).setProperty("class", this.style);
	...
	initialize: function(ParentTag, element){
		...
		//} else if(ParentTag == 'span'){
			...
			function saveSpan(){
				...
				actual_element.setProperty('name', $('prop_cf_text_field_name').value);
			}
			$('prop_cf_text').setStyle('display','block');
			$('prop_cf_text_text').value = actual_element.getText();
			$('prop_cf_text_field_name').value = actual_element.getProperty('name');

...
// 					if($kayitliaday[0]['ADI'] != trim(FormFactory::ucWordsTR($satir['adi'])) || $kayitliaday[0]['SOYADI'] != trim(FormFactory::toUpperCase($satir['soyadi'])) || $kayitliaday[0]['DOGUM_TARIHI'] != trim($satir['dogumtarihi']) || $kayitliaday[0]['DOGUM_YERI'] != trim(FormFactory::ucWordsTR($satir['dogumyeri'])) || $kayitliaday[0]['BABA_ADI'] != trim(FormFactory::ucWordsTR($satir['babaadi'])) || $kayitliaday[0]['CINSIYETI'] != $Cins || $kayitliaday[0]['EGITIMI'] != $egitimi || $kayitliaday[0]['CALISMA_DURUMU'] != $calismaDurum){
    ...
    function tckimlik($tckimlik)
    {
        ...
        if ($tckimlik[0] == 0 or !ctype_digit($tckimlik) or strlen($tckimlik) != 11) {
            ...
            if ($pest != $tckimlik[9] || $tumt % 10 != $tckimlik[10]) {
                return false;
            } else {
...
//                  $objPHPExcel = PHPExcel_IOFactory::load($files['upload']['tmp_name']);
            ...
            for ($i = 4; $i < (count($dataTest) + 4); $i++) {
                if ($adaylar[$i]['IBAN'] == "" and strlen($dataTest[$i]['IBAN']) == 26) {
                    $guncelle[$adaylar[$i]['TC_KIMLIK']]['IBAN'] = $dataTest[$i]['IBAN'];
                    $adaylar[$i]['IBAN'] = $dataTest[$i]['IBAN'];
                }
                if ($adaylar[$i]['TELEFON'] == "" and strlen($dataTest[$i]['TELEFON']) == 10) {
                    $guncelle[$adaylar[$i]['TC_KIMLIK']]['TELEFON'] = $dataTest[$i]['TELEFON'];
                    $adaylar[$i]['TELEFON'] = $dataTest[$i]['TELEFON'];
                }
                if ($adaylar[$i]['EMAIL'] == "" and $dataTest[$i]['EMAIL']!= "") {
                    $guncelle[$adaylar[$i]['TC_KIMLIK']]['EMAIL'] = $dataTest[$i]['EMAIL'];
                    $adaylar[$i]['EMAIL'] = $dataTest[$i]['EMAIL'];
                }
            }
...
//                }
                    ...
                    /////////////  EXCEL'DEN GELEN IBAN TELEFON EPOSTA BILGISINI GÜNCELLEME ////////////////
                    if  (count($guncelle[$satir['TC_KIMLIK']])>0){
                        $sql = "UPDATE M_BELGELENDIRME_OGRENCI SET IBAN=?, TELEFON = ?, EMAIL = ? WHERE TC_KIMLIK = ?";
                        $param = array(
                            $guncelle[$satir['TC_KIMLIK']]['IBAN'],
                            $guncelle[$satir['TC_KIMLIK']]['TELEFON'],
                            $guncelle[$satir['TC_KIMLIK']]['EMAIL'],
                            $satir['TC_KIMLIK']
                        );
